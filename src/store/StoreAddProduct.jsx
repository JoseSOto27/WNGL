...
const StoreAddProduct = () => {
  ...
  // ESTADO LIMPIO: Sin precio_mayorista
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      ...
      // INSERCIÓN SIN PRECIO_MAYORISTA
  ...
  return (
    <div className="p-4 sm:p-10 bg-white min-h-screen font-sans selection:bg-emerald-500 selection:text-white">
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="bg-[#0f1a04] rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden border-b-8 border-emerald-600">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8 relative z-10">
            <input
              ...
              className="flex-1 p-4 bg-white/5 rounded-xl font-black text-white uppercase italic text-xs outline-none border border-white/10 focus:border-emerald-500 transition-all"
            />
            ...
            {categorias.map(cat => (
              <div key={cat.id} className="flex items-center gap-3 bg-white/5 border border-white/5 pl-5 pr-3 py-2.5 rounded-full hover:border-emerald-500/50 transition-all group">
                {categoriaEditando === cat.id ? (
                  <input autoFocus value={nombreEditado} onChange={(e) => setNombreEditado(e.target.value)} className="bg-emerald-500/10 font-black italic uppercase outline-none w-24 text-[10px] text-emerald-400 p-1 rounded" />
                ) : <span className="font-black text-[10px] uppercase italic text-white/60 group-hover:text-white transition-colors">{cat.nombre}</span>}
                <div className="flex gap-1 border-l border-white/10 pl-3">
                  {categoriaEditando === cat.id ? (
                    <button onClick={() => guardarEdicionCat(cat.id)} className="text-emerald-400"><Check size={14} strokeWidth={3}/></button>
                  ) : (
                    <button onClick={() => {setCategoriaEditando(cat.id); setNombreEditado(cat.nombre)}} className="text-white/20 hover:text-emerald-500"><Pencil size={14}/></button>
                  )}
        ...
        <div className="bg-white rounded-[3rem] p-6 md:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <div className="space-y-4">
                ...
                <label className="relative h-80 lg:h-full min-h-[400px] bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 transition-all group overflow-hidden shadow-inner">
                  {imagen ? (
                    <img src={URL.createObjectURL(imagen)} className="object-contain w-full h-full p-8 group-hover:scale-105 transition-transform" alt="preview" />
                  ) : (
                    <div className="text-center p-6">
                      <div className="size-16 bg-white rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm group-hover:text-emerald-500 transition-colors">
                        <ImagePlus size={28} className="opacity-20 group-hover:opacity-100" />
                      </div>
                      <span className="text-[10px] font-black text-slate-300 uppercase italic tracking-widest">Cargar Perfil Visual</span>
                    </div>
                  )}
              ...
              <div className="lg:col-span-2 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    ...
                    <input name="nombre" value={infoProducto.nombre} onChange={manejarCambio} placeholder="EJ. WINGOOL MONSTER BURGER" className="w-full p-4 bg-slate-50 border-none rounded-2xl font-black text-[#0f1a04] uppercase italic text-xs outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all shadow-sm" required />
                  </div>
                  
                  <div className="md:col-span-2">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-4 mb-2 block italic tracking-widest">Descripción</label>
                    <textarea name="descripcion" value={infoProducto.descripcion} onChange={manejarCambio} placeholder="DESCRIBE TU PRODUCTO..." className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold text-slate-500 outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none italic text-xs shadow-sm" rows="3" />
                  </div>
                  ...
                  <div className="md:col-span-1">
                    ...
                    <select value={infoProducto.categoria} onChange={(e) => setInfoProducto({...infoProducto, categoria: e.target.value})} className="w-full p-4 bg-slate-50 border-none rounded-2xl font-black text-[#0f1a04] uppercase italic text-[11px] outline-none focus:ring-2 focus:ring-emerald-500/20 cursor-pointer shadow-sm" required>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 font-black text-xs italic">$</span>
                        <input type="number" name="precio_original" step="0.01" value={infoProducto.precio_original} onChange={manejarCambio} className="w-full p-4 pl-8 bg-slate-50 border-none rounded-2xl font-black text-slate-400 text-center text-xs outline-none shadow-sm" required />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-500 font-black text-xs italic">$</span>
                        <input type="number" name="precio_offer" step="0.01" value={infoProducto.precio_oferta} onChange={(e) => setInfoProducto({...infoProducto, precio_oferta: e.target.value})} className="w-full p-4 pl-8 bg-emerald-50 border-2 border-emerald-100 rounded-2xl font-black text-emerald-600 text-center text-sm outline-none italic shadow-md" required />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#0f1a04] rounded-[2.5rem] p-8 shadow-xl space-y-6 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center relative z-10">
                      <div className="flex items-center gap-3">
                        <div className="bg-emerald-500/10 p-2 rounded-xl text-emerald-500"><Coins size={18}/></div>
                        <h3 className="font-[1000] text-white text-[10px] uppercase italic tracking-widest leading-none">Gestionar Extras MVP</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-64 overflow-y-auto custom-scrollbar pr-2 relative z-10">
                      {infoProducto.ingredientes.map(item => (
                        <div key={item.id} className="flex gap-3 items-center bg-white/5 p-4 rounded-2xl border border-white/5 group hover:border-emerald-500/30 transition-all">
                          ...
                          <div className="flex items-center gap-1.5 bg-emerald-500 rounded-xl px-3 py-1.5">
                            <span className="text-[#0f1a04] text-[10px] font-black italic">$</span>
                            <input type="number" step="0.01" value={item.precio} onChange={(e) => manejarCambioExtra(item.id, 'precio', e.target.value)} className="w-10 bg-transparent text-[#0f1a04] text-right text-[11px] font-[1000] italic outline-none" />
                          </div>
