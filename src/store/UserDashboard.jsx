...
import { 
  ...
  X, Package, Receipt, Truck, UtensilsCrossed, Wallet, Ticket, Flame, ChevronRight
} from "lucide-react";
...
const UserDashboard = () => {
  ...
  // CONFIGURACIÓN DE ESTATUS ULTRA-ENTENDIBLE
  const statusConfig = {
    pendiente: { 
      step: 1, 
      label: "PEDIDO RECIBIDO", 
      desc: "Estamos confirmando tu orden",
    ...
    procesando: { 
      step: 2, 
      label: "PREPARANDO", 
      desc: "Tu comida está en la cocina",
      color: "text-blue-500", 
      bg: "bg-blue-500/10", 
      icon: <Flame size={20} className="animate-bounce" /> 
    },
    enviado: { 
      step: 3, 
      label: "EN CAMINO", 
      desc: "El repartidor va hacia ti",
              ...
              {orders.length > 0 ? orders.map((order) => {
                ...
                return (
                  <div 
                    ...
                    className="p-6 flex items-center justify-between border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-all active:scale-[0.98] group"
                  >
                    <div className="flex items-center gap-5">
                      <div className={`p-3 rounded-2xl ${config.bg} ${config.color} transition-all duration-500`}>
                        {config.icon}
                      </div>
                      <div>
          ...
          <div className="bg-white w-full max-w-lg rounded-[3.5rem] overflow-hidden shadow-2xl animate-in zoom-in duration-300 border-b-[16px] border-emerald-500 relative">
            ...
            <div className="p-8 space-y-6 max-h-[65vh] overflow-y-auto custom-scrollbar">
                  ...
                  {selectedOrder.productos?.map((prod, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="text-[11px] font-black text-[#1a2e05] uppercase italic">{prod.nombre}</p>
                        </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-[#1a2e05] p-8 rounded-[3rem] space-y-3 shadow-xl text-white">
                <div className="flex justify-between text-[10px] font-black text-white/40 uppercase italic tracking-widest">
                  <span>Subtotal Comida</span>
                  <span className="text-white">${selectedOrder.subtotal || (selectedOrder.total - 40 + (selectedOrder.puntos_usados || 0))}</span>
                </div>
                <div className="flex justify-between text-[10px] font-black text-white/40 uppercase italic tracking-widest">
                ...
                {selectedOrder.puntos_usados > 0 && (
                  <div className="flex justify-between text-[11px] font-black text-emerald-400 uppercase italic bg-emerald-500/10 p-3 rounded-2xl border border-emerald-500/20 shadow-lg">
                    <span className="flex items-center gap-2"><Wallet size={12}/> Wingool Wallet (Puntos)</span>
                    <span>-${selectedOrder.puntos_usados}</span>
                  </div>
                )}

                <div className="flex justify-between items-end pt-5 border-t border-white/10 mt-2">
                  <div className="leading-none">
                    <span className="text-[10px] font-black uppercase italic text-emerald-500 tracking-widest">Total Pagado</span>
                    <p className="text-[8px] font-bold text-white/20 uppercase mt-1 italic leading-none">Vía {selectedOrder.metodo_pago}</p>
                  </div>
              ...
              <div className="bg-emerald-500 p-5 rounded-[2.5rem] text-white flex justify-between items-center shadow-lg transform active:scale-95 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                      <p className="text-[10px] font-black uppercase tracking-widest italic leading-none">Puntos Ganados</p>
                      <p className="text-[8px] font-bold text-white/60 uppercase mt-1">Sumados a tu Wallet</p>
                  </div>
                </div>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-3">
                 ...
                 <div>
                    <p className="text-[8px] font-black text-slate-300 uppercase italic mb-0.5 tracking-widest">Entregar en:</p>
                    <p className="text-[10px] font-[1000] text-slate-600 uppercase italic tracking-tighter leading-none">{selectedOrder.direccion_entrega}</p>
                 </div>
              </div>
