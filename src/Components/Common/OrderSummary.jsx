...
import { ShoppingBag, CheckCircle, Loader2, Zap, CreditCard, Wallet, MapPin, Ticket, ReceiptText, User, Info, XCircle, AlertTriangle, RefreshCw } from "lucide-react";
    ...
    const handlePagoExitoso = async () => {
        try {
            const { error } = await supabase.from('pedidos_v2').insert([{
                ...
                // CORRECCIÓN: Se incluyen ingredientes y subtotal por cada ítem
            ...
            toast.error("Error en el registro del pago");
        }
    };
        ...
        return (
            <div className="w-full max-w-md mx-auto bg-white p-12 rounded-[4rem] shadow-2xl text-center border-4 border-red-500 animate-in zoom-in duration-500 relative overflow-hidden">
                ...
                <button 
                    ...
                    className="w-full bg-red-500 text-white py-6 rounded-[2rem] font-[1000] uppercase italic flex items-center justify-center gap-3 hover:bg-red-600 transition-all shadow-xl active:scale-95 mb-4"
                >
                    <RefreshCw size={20} /><span>REINTENTAR PAGO</span>
                </button>
                <button 
                    onClick={() => { setPaymentError(false); setMetodoPago("efectivo"); }} 
                    className="text-[10px] font-[1000] text-slate-400 uppercase italic tracking-widest hover:text-[#1a2e05]"
                >
                    CAMBIAR A EFECTIVO
                </button>
            </div>
    ...
    return (
        <div className="w-full max-w-md mx-auto bg-white p-8 rounded-[4rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100 space-y-6 relative">
            ...
            <div className="space-y-4">
                ...
                <button 
                    onClick={() => { if (!selectedAddress) return toast.error("Selecciona una dirección"); metodoPago === "tarjeta" ? setMostrarPasarela(true) : handlePedidoEfectivo(); }} 
